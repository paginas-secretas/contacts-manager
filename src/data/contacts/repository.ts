import { DocumentHash, DocumentReference, EncryptedContactsList } from "../model";
...
export interface ContactsRepository {
    fetch(ref: DocumentReference, hash: DocumentHash): Promise<EncryptedContactsList>
}
...
export class GitHubContactsRepository implements ContactsRepository {
    ...
    async fetch(ref: string, hash: string): Promise<EncryptedContactsList> {
        const endpointUrl = new URL(`/${this.config.owner}/${this.config.repo}/${hash}/${ref}`, rawGitHubServerBaseUrl);
        ...
        return requestBodyToEncryptedContactsList(
